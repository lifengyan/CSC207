...
/**
 * PickerManager stores information of pickers who are currently working on a request 
 * into pickerList and pickers who are currently free into freePicker list. Once pickers finished 
 * their picking request, they will be removed from the pickerList since no further action
 * is needed form them. Once free pickers are assigned with picking requests, they will be 
 * removed from freePicker list
 * @author TongZhu
 *
 */
public class PickerManager {
  private ArrayList<Picker> pickerList = new ArrayList<Picker>(); // store currently working and free pickers
  private ArrayList<Picker> freePicker = new ArrayList<Picker>(); // store free pickers

  /**
   * use input string of a picker's name to get the Picker instance associated with this name, 
   * if no such picker exist in the list, then create a new Picker instance with give string name 
   * and put it in the pickerList
  ...
  /**
   * getter function that returns an Picker type arrayList of ready/free pickers, the Picker at index 0 
   * should be the first one to receive next picking request
